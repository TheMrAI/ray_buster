...
auto translate::hit(ray const& incoming_ray, double t_min, double t_max, hit_record& rec) const -> bool {
  ...
  if(!object_->hit(translated_ray, t_min, t_max, rec)) {
    return false;
  }
  rec.p += offset_;
...
auto translate::bounding_box(double time_0, double time_1, aabb& bounding_box) const -> bool {
  if(!object_->bounding_box(time_0, time_1, bounding_box)) {
    return false;
  }
...
rotate_y::rotate_y(std::shared_ptr<hittable> object, double angle): object_{object} {
  ...
  vec3 lower_bound{ std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity()};
  vec3 upper_bound{-std::numeric_limits<double>::infinity(), -std::numeric_limits<double>::infinity(), -std::numeric_limits<double>::infinity()};
...
auto rotate_y::hit(ray const& incoming_ray, double t_min, double t_max, hit_record& rec) const -> bool {
  ...
  if(!object_->hit(rotated_ray, t_min, t_max, rec)) {
    return false;
  }
