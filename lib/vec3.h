...
public:
    ...
    double Length() const {
        return lina::length(v_);
    }

    std::array<double, 3> const & Components() const {
        return v_;
    }
private:
...
Vec3 operator*(Vec3 const& lhs, double scalar) {
    return Vec3{lina::scale(scalar, lhs.Components())};
}

Vec3 operator*(double scalar, Vec3 const& lhs) {
    return Vec3{lina::scale(scalar, lhs.Components())};
}

Vec3 operator/(Vec3 const& lhs, double scalar) {
    return Vec3{lina::scale(1.0/scalar, lhs.Components())};
}

double dot(Vec3 const& lhs, Vec3 const& rhs) {
    return lina::dot(lhs.Components(), rhs.Components());
}

Vec3 cross(Vec3 const& lhs, Vec3 const& rhs) {
    return Vec3{lina::cross(lhs.Components(), rhs.Components())};
}

Vec3 unit(Vec3 const& vector) {
    return Vec3{lina::unit(vector.Components())};
}
